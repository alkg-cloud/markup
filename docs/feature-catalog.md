...
| `mockup-viewer-mockup-doc` | The iframe rendering `/m/[id]/...`. Receives `transform: scale(var(--zoom))` for canvas zoom. Iframe content is **interactive outside draft mode** — clicks pass through to embedded buttons, links, and overlays (e.g. the design-feature tweaker on idea mockups). While a draft is open, empty-area clicks are captured as pin anchors via `useViewerCanvas` and `preventDefault`'d; existing-pin clicks always route to the rail regardless of draft state. | loading, loaded, error; interactive (default), pin-capture (while drafting) |
