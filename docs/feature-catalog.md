...
| `project-card-kebab-menu` | Native HTML popover (`popover="auto"`) with the three actions above. Light-dismiss + ESC handled by the browser. The Delete item fires `confirm-dialog` and on accept calls `DELETE /api/projects/[id]`, then refreshes the page list. **Delete is hidden for non-admin viewers when the project's `(createdBy, createdByType)` does not match the viewer's identity (or is `null`). Admins see Delete on every card.** See `[fc:delete-button-gating]`. | open, closed |
...
Inline diff modal and edit-mode toggle are parked — see
...
| `viewer-iframe-viewport-fit` | Fit mode (default) sizes the iframe with `width: 100%; height: 100%` — no backdrop, no shadow, no handles. | default state on first mockup visit |
...
| `mockup-viewer-rail-add-button` | "+ New annotation" button at foot. Morphs round → pill with label + `<Kbd keys={['mod','shift','n']} />` keycap chip (OS-aware rendering). `aria-label` uses `formatShortcut` for plain-text SR announcement. Pinned via `flex-shrink: 0` to the spec-default 32 × 32 pill so the inner 30 × 30 icon sits dead-centre even though the collapsed foot inner is narrower than the button's declared width. | collapsed/expanded width transition, hidden (read-only-historic) |
| `mockup-viewer-rail-read-only-historic` | Whole rail enters this mode when the parent `AppMainViewer` is in historic version viewing (URL `?v=<vid>`). The "+ New annotation" button is removed and the draft slot is not rendered; badges, expanded cards, lock toggle, drag handle remain active so read-only navigation is preserved. | hidden, active |
...
| `mockup-viewer-version-item` | One row per version (dot + label + sub). Rendered by the `VersionListRow` subcomponent so each row carries its own `usePopover` for the per-row kebab; nested popovers stack natively via the HTML spec's ancestor relationship — opening a row kebab does NOT close the parent version list. Clicking a non-current row enters historic viewing mode (URL `?v=<vid>`, banner + read-only rail); clicking the current row exits. | default, current (accent bg + glowing dot) |
...
| `mockup-viewer-version-action-delete` | Per-row "Delete" item (label dropped the redundant noun) — opens a `confirm-dialog` quoting the version label (e.g. "Delete v3") + warning that anchors pointing at this version lose their source-of-truth pin. DELETE `/api/mockups/[id]/versions/[vid]` only after confirm. **Hidden for non-admin viewers when the version's `(createdBy, createdByType)` does not match the viewer's identity.** See `[fc:delete-button-gating]`. | danger styling |
| `mockup-viewer-historic-banner` | Glass pill at top-center inside the canvas (under the toolbar) shown only when the URL carries `?v=<vid>` and the vid is a known non-current version. Content: history icon + "Viewing vN" + "Back to vM (current)" button. Clicking the button or the current row in the version popover calls `router.replace(pathname)` and dismisses the banner. Enter animation: fade + slide-down using `--motion-fast` / `--ease-standard`; respects `prefers-reduced-motion`. Outer wrapper is `role="status" aria-live="polite"`. EN copy. | hidden, visible |
...
| ID | Surface / Interaction | States |
|---|---|---|
| `annotation-card` | Card with meta + primary comment + foot + chevron thread | default, active (accent bg 30% + left stripe), hover, read-only-historic (no primary kebab, no reply form, no Add reaction, existing reactions visible but non-toggleable) |
...
| `agent-tokens` | Each agent token renders as a card pixel-aligned with DS 10-agent-tokens V3: 600 px max-width page container, 36×36 key-icon box (inline SVG, accent-soft bg + `--accent` tinted), name (14 px / 600), meta line `Created X · Last used Y` (or `never`) in `--text-muted`, masked token preview `mk_{live|test}_••••••••••••<lastFour>` in `--font-mono` inside an `--bg-elevated` chip with `--border-subtle` and `--radius-xs`, and two icon buttons — `CopyButton variant="icon"` (copies masked preview) + revoke button, each 28×28. Topbar of the page renders an empty breadcrumb — the page h1 already carries the label. | default; copy: standard `copy-button-icon` states; revoke hover: `--danger-soft` bg + `--danger` color |
...
| `settings-agents-plaintext` | One-time plaintext token display with `CopyButton variant="secondary" feedback="both"` shown immediately after creation. Toast message: "Token copied — keep it safe". | shown once after creation; copied state (1200 ms inline + toast) |
...
| `settings-invites-copy-btn` | `CopyButton variant="icon"` per unused card. Disabled when no session plaintext available (link is only available right after creation). On click writes `${APP_URL}/invite/<token>` to the clipboard via `useCopy`, swaps icon to check for 1200 ms, fires toast `Invite link copied` (3 s). | default, hover, copied (check icon swap + accent tint, 1200 ms), error (shake + "Copy failed" toast), disabled (no plaintext), focus-visible |
...
Copy-to-clipboard button (`CopyButton.tsx` + `useCopy.ts`). Four variants: icon (default 28×28), ghost, secondary, accent. Custom `copied` and `error` states (not Radix primitives). `useCopy` hook also exportable for non-button affordances. DS source: `docs/design/design-system/30-copy-button.html`.

| ID | Surface / Interaction | States |
|---|---|---|
| `copy-button-icon` | Default 28×28 icon-only button (`variant="icon"`). Copy icon (VscCopy) at rest; check icon (VscCheck) while copied. Icon-only always fires a toast on success (SR rule). | default, hover (`--surface-hover`), copied (`data-state="copied"`, 1200 ms, accent tint + check icon + `iconPop` animation), error (`data-state="error"`, shake animation, always fires "Copy failed" toast), disabled (opacity 0.4), focus-visible |
| `copy-button-ghost` | `variant="ghost"` — auto-width, 2 px 6 px padding, 12 px text. | default, hover, copied, error, disabled, focus-visible |
| `copy-button-secondary` | `variant="secondary"` — 28 px tall, `--btn-bg` surface, `--border`, label. Label text swaps to "Copied!" when copied. | default, hover, copied, error, disabled, focus-visible |
| `copy-button-accent` | `variant="accent"` — 28 px tall, `--accent-soft` surface, `--accent-overlay-mid` border, label. | default, hover, copied, error, disabled, focus-visible |
| `anim-copy-feedback` | `iconPop` animation on the check icon when copied state activates. `copyShake` on the root button when error state activates. Both respond to `prefers-reduced-motion`. | animated on state change |
...
|---|---|---|---|
| `agent-annotation-create` | Annotation creation: JSON body with `body` + `anchors[]` + `colorIndex` + `status` for the comment-flow (drives `draft-card`) | `draft-card` | `POST /api/mockups/[id]/annotations` |
