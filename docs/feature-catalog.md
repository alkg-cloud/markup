...
| `new-mockup-dialog-project-picker` | Project selector — native `<select>` in the default layout. Pre-selected by view inference (current project, or `Unsorted` at `/`) | default, focused, changed (re-fetches the folder tree for the new project and resets the folder picker) |
| `new-mockup-dialog-folder-picker` | Folder selector with depth indentation (`Hero`, `Hero / Section`, `Hero / Section / Variants`). Pre-selected by view inference. Native `<select>` or inline option-list per layout variant. Shows `Loading folders…` (disabled) while the per-project tree is in flight | default, focused, depth-indented options, loading (folder tree request in flight) |
