...
function DiffModal({ diffText, onClose }: { diffText: string | null; onClose: () => void }) {
  return createElement(
    ...
    createElement('button', { type: 'button', 'aria-label': 'Close diff', onClick: onClose }, '✕'),
...
function DiffWrapper({
  ...
  return createElement(
    ...
    createElement(
      'button',
      { type: 'button', 'data-testid': 'open-diff', onClick: onDiff },
      'Diff',
    ),
    diffOpen
...
describe('Diff handler — multiple versions', () => {
  ...
  it("shows 'Loading diff…' before fetch resolves then shows result", async () => {
    ...
    const pendingText = new Promise<string>((res) => {
      resolveText = res;
    });
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
